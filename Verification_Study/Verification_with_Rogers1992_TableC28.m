%% Verification with Rogers (1992) table C-28
...
%% INPUT
load('./Data/VerData_Rogers1992_TableC28')     % format string
...
fid = fopen('Verification_Rogers1992_TableC28.tex','w');
fprintf(fid, '%%%%%%%%%% TABLE PRINTED BY MATLAB PROGRAM %%%%%%%%%%\n');
fprintf(fid, '%%%% Verification with Table C-28 from Rogers\\cite{rogers1992laminar}: Pr=0.723, C=1 (constant), sigma=2.0 (Ma=inf)\n');
% fprintf(fid, '%%\\begin{table}\n');
% fprintf(fid, '%%\\begin{sidewaystable}\n');
fprintf(fid, '\\begin{threeparttable}\n');
fprintf(fid, '    \\centering\n');
fprintf(fid, '    \\caption{Comparison with tabulated data of adiabatic laminar compressible similar flows with constant nonzero pressure gradients for calorically perfect ideal gas with $C = 1$ (constant) and $\\mathrm{Pr} = 0.723$ and $\\bar{\\sigma} = 2.0 \\quad (\\mathrm{Ma} = \\infty)$ taken from Rogers\\cite{rogers1992laminar} table C-28. The values obtained with the CS-method (CSM) are transformed from the compressible Falkner-Skan transformed y-coordinate with uniform (vertical) grid spacing of $\\mathrm{d} \\eta = \\sqrt{\\frac{2C}{m_2 + 1}} %1.4f$ and height of $\\eta_{\\mathrm{e}} = \\sqrt{\\frac{2C}{m_2 + 1}} %1.1f$ to the Illingworth-Levy coordinates ($\\mathrm{d} \\eta = %1.4f$ and $\\eta_{\\mathrm{e}} = %1.1f$). Note that separation occurred when the table entry shows ''sep''.}\n',GRD.Deta(1),GRD.etaE,GRD.Deta(1),GRD.etaE);
fprintf(fid, '    \\label{tab:C28}\n');
fprintf(fid, '    \\begin{tabular}{S[table-format=-1.6] S[table-format=-1.6e-1] S[table-format=1.6] S[table-format=1.6] S[table-format=-1.6] S[table-format=-1.6]}\n');
fprintf(fid, '        \\toprule\n\n');
fprintf(fid, '        \\multicolumn{1}{c}{$\\hat{\\beta}$}                         &\n');
fprintf(fid, '        \\multicolumn{1}{c}{$m_2$}                                 &\n');
fprintf(fid, '        \\multicolumn{2}{c}{$g_{\\mathrm{aw}}$}                     &\n');
fprintf(fid, '        \\multicolumn{2}{c}{$f''''(0)$}                              \\\\\n');
fprintf(fid, '        \\cmidrule(lr){3-4} \\cmidrule(lr){5-6}\n\n');
...
%     end
    if sep(ll) == 1 % separation has occurred, print 'sep'
        fprintf(fid, '         %s       &      %s               &  %s   &  \\multicolumn{1}{c}{sep}   &  %s   &  \\multicolumn{1}{c}{sep}  \\\\\n',VerData_Rogers1992_TableC28(mm,1),num2str(PGm2(mm),'%1.6e'),VerData_Rogers1992_TableC28(mm,2),VerData_Rogers1992_TableC28(mm,3));
    else
        fprintf(fid, '         %s       &      %s               &  %s   &  %1.6f   &  %s   &  %1.6f  \\\\\n',VerData_Rogers1992_TableC28(mm,1),num2str(PGm2(mm),'%1.6e'),VerData_Rogers1992_TableC28(mm,2),CSM_DataTableC28(ll,2),VerData_Rogers1992_TableC28(mm,3),CSM_DataTableC28(ll,1)*sqrt(2*FLD.C/(PGm2(mm) + 1)));
    end
end
% fprintf(fid, '        %% Data\n');
...
%% Table C-28 extended (J1, J2) - Write data to Tex-file (in Table format) #1

% % % NewFolder = cd;
% cd('Thesis_Report_Tables') %%% new, added (21-01-2021)
fid = fopen('Verification_Rogers1992_TableC28_extended.tex','w');
fprintf(fid, '%%%%%%%%%% TABLE PRINTED BY MATLAB PROGRAM %%%%%%%%%%\n');
fprintf(fid, '%%%% Verification with Table C-28 from Rogers\\cite{rogers1992laminar}: Pr=0.723, C=1 (constant), sigma=2.0 (Ma=inf)\n');
% fprintf(fid, '%%\\begin{table}\n');
% fprintf(fid, '%%\\begin{sidewaystable}\n');
...
fprintf(fid, '    \\centering\n');
fprintf(fid, '    \\caption{Comparison with tabulated data of adiabatic laminar compressible similar flows with constant nonzero pressure gradients for calorically perfect ideal gas with $C = 1$ (constant) and $\\mathrm{Pr} = 0.723$ and $\\bar{\\sigma} = 2.0 \\quad (\\mathrm{Ma} = \\infty)$ taken from Rogers\\cite{rogers1992laminar} table C-28. The values obtained with the CS-method (CSM) are transformed from the compressible Falkner-Skan transformed y-coordinate with uniform (vertical) grid spacing of $\\mathrm{d} \\eta = \\sqrt{\\frac{2C}{m_2 + 1}} %1.4f$ and height of $\\eta_{\\mathrm{e}} = \\sqrt{\\frac{2C}{m_2 + 1}} %1.1f$ to the Illingworth-Levy coordinates ($\\mathrm{d} \\eta = %1.4f$ and $\\eta_{\\mathrm{e}} = %1.1f$). Note that separation occurred when the table entry shows ''sep''.}\n',GRD.Deta(1),GRD.etaE,GRD.Deta(1),GRD.etaE);
fprintf(fid, '    \\label{tab:C28E}\n');
fprintf(fid, '    \\begin{tabular}{S[table-format=-1.6] S[table-format=-1.6e-1] S[table-format=1.6] S[table-format=1.6] S[table-format=-1.6] S[table-format=-1.6] S[table-format=1.6] S[table-format=1.6] S[table-format=1.6] S[table-format=1.6]}\n');
fprintf(fid, '        \\toprule\n\n');
fprintf(fid, '        \\multicolumn{1}{c}{$\\hat{\\beta}$}                         &\n');
fprintf(fid, '        \\multicolumn{1}{c}{$m_2$}                                 &\n');
fprintf(fid, '        \\multicolumn{2}{c}{$g_{\\mathrm{aw}}$}                     &\n');
...
%     elseif Beta_dact(mm) == 2.0
%     if Beta_dact(mm) == 2.0
%         fprintf(fid, '         %s       &   %s       &   %4.0f\\tnote{1}          &  %s   &  %1.6f   &  %s   &  %1.6f   &  %s   &  %1.6f   &  %s   &  %1.6f  \\\\\n',VerData_Rogers1992_TableC28(mm,1),VerData_Rogers1992_TableC28(mm,2),PGm2(mm),VerData_Rogers1992_TableC28(mm,3),CSM_DataTableC28(ll,1)*sqrt(2*FLD.C/(PGm2(mm) + 1)),VerData_Rogers1992_TableC28(mm,4),CSM_DataTableC28(ll,2)*sqrt(2*FLD.C/(PGm2(mm) + 1)),VerData_Rogers1992_TableC28(mm,5),CSM_DataTableC28(ll,3)/sqrt(2*FLD.C/(PGm2(mm) + 1)),VerData_Rogers1992_TableC28(mm,6),CSM_DataTableC28(ll,4)/sqrt(2*FLD.C/(PGm2(mm) + 1)));
%     elseif sep(ll) == 1 % separation has occurred, print 'sep'
    if sep(ll) == 1 % separation has occurred, print 'sep'
        fprintf(fid, '         %s       &      %s               &  %s   &  \\multicolumn{1}{c}{sep}   &  %s   &  \\multicolumn{1}{c}{sep}   &  %s   &  \\multicolumn{1}{c}{sep}   &  %s   &  \\multicolumn{1}{c}{sep}  \\\\\n',VerData_Rogers1992_TableC28(mm,1),num2str(PGm2(mm),'%1.6e'),VerData_Rogers1992_TableC28(mm,2),VerData_Rogers1992_TableC28(mm,3),VerData_Rogers1992_TableC28(mm,4),VerData_Rogers1992_TableC28(mm,5));
    else
        fprintf(fid, '         %s       &      %s               &  %s   &  %1.6f   &  %s   &  %1.6f   &  %s   &  %1.6f   &  %s   &  %1.6f  \\\\\n',VerData_Rogers1992_TableC28(mm,1),num2str(PGm2(mm),'%1.6e'),VerData_Rogers1992_TableC28(mm,2),CSM_DataTableC28(ll,2),VerData_Rogers1992_TableC28(mm,3),CSM_DataTableC28(ll,1)*sqrt(2*FLD.C/(PGm2(mm) + 1)),VerData_Rogers1992_TableC28(mm,4),CSM_DataTableC28(ll,3)*sqrt(2*FLD.C/(PGm2(mm) + 1)),VerData_Rogers1992_TableC28(mm,5),CSM_DataTableC28(ll,4)*sqrt(2*FLD.C/(PGm2(mm) + 1)));
    end
end
% fprintf(fid, '        %% Data\n');
fprintf(fid, '        \\bottomrule\n\n');
fprintf(fid, '    \\end{tabular}\n');
% fprintf(fid, '    \\begin{tablenotes}\n');
% fprintf(fid, '        The solution does not converge for $m_2 = \\infty$, and therefore $m_2 = 1000$ is taken as approach.\n');
% fprintf(fid, '    \\end{tablenotes}\n');
fprintf(fid, '\\end{threeparttable}\n');
% fprintf(fid, '%%\\end{sidewaystable}\n');
% fprintf(fid, '%%\\end{table\n');
...
%% Table C-28 extended - Appendix - (J1, J2) - Write data to Tex-file (in Table format) #1
% Two changes (21-01-2022):
% 1) added '_A' to name
% 2) changed label to 'tab:C28EA' ('A' added)
% This prevents references crossing/error

% % % NewFolder = cd;
% cd('Thesis_Report_Tables') %%% new, added (21-01-2021)
fid = fopen('Verification_Rogers1992_TableC28_extended_A.tex','w');
fprintf(fid, '%%%%%%%%%% TABLE PRINTED BY MATLAB PROGRAM %%%%%%%%%%\n');
fprintf(fid, '%%%% Verification with Table C-28 from Rogers\\cite{rogers1992laminar}: Pr=0.723, C=1 (constant), sigma=2.0 (Ma=inf)\n');
% fprintf(fid, '%%\\begin{table}\n');
% fprintf(fid, '%%\\begin{sidewaystable}\n');
fprintf(fid, '\\begin{threeparttable}\n');
fprintf(fid, '    \\centering\n');
fprintf(fid, '    \\caption{Repeated Table \\ref{tab:C28E}}: Comparison with tabulated data of adiabatic laminar compressible similar flows with constant nonzero pressure gradients for calorically perfect ideal gas with $C = 1$ (constant) and $\\mathrm{Pr} = 0.723$ and $\\bar{\\sigma} = 2.0 \\quad (\\mathrm{Ma} = \\infty)$ taken from Rogers\\cite{rogers1992laminar} table C-28. The values obtained with the CS-method (CSM) are transformed from the compressible Falkner-Skan transformed y-coordinate with uniform (vertical) grid spacing of $\\mathrm{d} \\eta = \\sqrt{\\frac{2C}{m_2 + 1}} %1.4f$ and height of $\\eta_{\\mathrm{e}} = \\sqrt{\\frac{2C}{m_2 + 1}} %1.1f$ to the Illingworth-Levy coordinates ($\\mathrm{d} \\eta = %1.4f$ and $\\eta_{\\mathrm{e}} = %1.1f$). Note that separation occurred when the table entry shows ''sep''.}\n',GRD.Deta(1),GRD.etaE,GRD.Deta(1),GRD.etaE);
fprintf(fid, '    \\label{tab:C28E_A}\n');
fprintf(fid, '    \\begin{tabular}{S[table-format=-1.6] S[table-format=-1.6e-1] S[table-format=1.6] S[table-format=1.6] S[table-format=-1.6] S[table-format=-1.6] S[table-format=1.6] S[table-format=1.6] S[table-format=1.6] S[table-format=1.6]}\n');
fprintf(fid, '        \\toprule\n\n');
fprintf(fid, '        \\multicolumn{1}{c}{$\\hat{\\beta}$}                         &\n');
fprintf(fid, '        \\multicolumn{1}{c}{$m_2$}                                 &\n');
