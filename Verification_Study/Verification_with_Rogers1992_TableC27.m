%% Verification with Rogers (1992) table C-27
% Nonzero pressure gradient including heat transfer for Pr=0.723, C=1 (C needs to be a constant), Ma=inf (sigma=2), calorically perfect ideal gas.
% Similar solutions
% 28-09-2019 D.D.D.
% last changes on 24-10-2019
% last run on 04-05-2020
% Simulation takes about 16 minutes
% Adapted slightly for upload and checked: 21-01-2022

%% Note: only for single station calculations
%%% note that this version is changed compared to C-25 and C-26: with Ma>0
%%% string-replace is used to print scientific notation (for small and large m2);
%%% for m2=1000 strrep is changed to substitute for a + (instead of - in
%%% the other cases)
%%% NB later strrep is replaced by: 'num2str(PGm2(mm),'%1.6e')' together
%%% with formatting of SI-unitx table column: ''
...
%% INPUT
load('./Data/VerData_Rogers1992_TableC27')          % format string
% INPUT_Verification_with_Rogers1992                  % case specific INPUT (options and settings)
run('./INPUT/INPUT_Verification_with_Rogers1992')   % case specific INPUT (options and settings)

% Change of input (compare with Table C-25 and C-26, where INPUT-file is standardized for case C-25):
FLD.Pr = 0.723;
INP.MaI = 1000; % since not possible to simulate Mach = inf
INP.MaE = INP.MaI;

% Settings
SET.ITMAX = 100;40; % gw=0, Beta_dact=2.00 takes already 33 iterations

%% Initialization
Size = length(VerData_Rogers1992_TableC27);
Beta_dact = zeros(Size,1);
PGm2 = zeros(Size,1);
gw = zeros(Size,1);
% CSM_DataTableC27 = NaN*ones(Size,2);
CSM_DataTableC27 = NaN*ones(Size,6); %%% f''(0), g'(0) or g(0), J1, J2, J3, J4
sep = NaN*ones(Size,1);
kk = NaN*ones(Size,1);
NS = 1;
NST = length(INP.x);
...
for jj = 1:Size
    ...
    Beta_dact(jj) = str2double(VerData_Rogers1992_TableC27(jj,2));
    gw(jj) = str2double(VerData_Rogers1992_TableC27(jj,1));
    ...
    if jj > 1 && (Beta_dact(jj) > Beta_dact(jj - 1) && sign(Beta_dact(jj)) == sign(Beta_dact(jj - 1))) %|| gw(jj) == 1.0 % 1) 2nd solution, %%% not applicable here, since Pr not equal to 1 and Ma not equal to zero: or 2) since already taken into account by other table (Falkner-Skan)
        % skip line
%         if gw(jj) == 1.0 %%% not the case for Table C-27
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        if MON.SEP == 1
            CSM_DataTableC27(ii,1) = NaN;
            CSM_DataTableC27(ii,2) = NaN;
            CSM_DataTableC27(ii,3) = NaN; % #1
            CSM_DataTableC27(ii,4) = NaN; % #1
            CSM_DataTableC27(ii,5) = NaN; % #1
            CSM_DataTableC27(ii,6) = NaN; % #1
            sep(ii,1) = 1;
        else
            CSM_DataTableC27(ii,1) = SOL{end}.v(1); % storing shear parameter; or in transformed form: SOL{end}.v(1)*sqrt(2/(m+1));
            CSM_DataTableC27(ii,2) = SOL{end}.p(1); % storing HT parameter; or in transformed form: SOL{end}.p(1)*sqrt(2/(m+1));
            ...
            CSM_DataTableC27(ii,3) = J1;
            CSM_DataTableC27(ii,4) = J2;
            CSM_DataTableC27(ii,5) = J3;
            CSM_DataTableC27(ii,6) = J4;
...
%% Table C-27 - Write data to Tex-file (in Table format)

NewFolder = cd; % currently in 'boundary-layer-code'-folder
cd('Thesis_Report_Tables') %%% new, added (21-01-2021)
fid = fopen('Verification_Rogers1992_TableC27.tex','w');
fprintf(fid, '%%%%%%%%%% TABLE PRINTED BY MATLAB PROGRAM %%%%%%%%%%\n');
fprintf(fid, '%%%% Verification with Table C-27 from Rogers\\cite{rogers1992laminar}: Pr=0.723, C=1, sigma=2.0 (Ma=inf) (constant)\n');
% fprintf(fid, '%%\\begin{table}\n');
% fprintf(fid, '%%\\begin{sidewaystable}\n');
fprintf(fid, '\\begin{threeparttable}\n');
fprintf(fid, '    \\centering\n');
fprintf(fid, '    \\caption{Comparison with tabulated data of laminar compressible similar flows with constant nonzero pressure gradients and heat transfer for calorically perfect ideal gas with $C = 1$ (constant) and $\\mathrm{Pr} = 0.723$ and $\\bar{\\sigma} = 2.0 \\quad (\\mathrm{Ma} = \\infty)$ taken from Rogers\\cite{rogers1992laminar} table C-27. The values obtained with the CS-method (CSM) are transformed from the compressible Falkner-Skan transformed y-coordinate with uniform (vertical) grid spacing of $\\mathrm{d} \\eta = \\sqrt{\\frac{2C}{m_2 + 1}} %1.4f$ and height of $\\eta_{\\mathrm{e}} = \\sqrt{\\frac{2C}{m_2 + 1}} %1.1f$ to the Illingworth-Levy coordinates ($\\mathrm{d} \\eta = %1.4f$ and $\\eta_{\\mathrm{e}} = %1.1f$). Note that separation occurred when the table entry shows ''sep''.}\n',GRD.Deta(1),GRD.etaE,GRD.Deta(1),GRD.etaE);
fprintf(fid, '    \\label{tab:C27}\n');
% fprintf(fid, '    \\begin{tabular}{S[table-format=1.2] S[table-format=2.6] S[table-format=4.6] S[table-format=2.6] S[table-format=2.6] S[table-format=2.6] S[table-format=2.6]}\n');
fprintf(fid, '    \\begin{tabular}{S[table-format=1.1] S[table-format=-1.6] S[table-format=-1.6e-1] S[table-format=-1.6] S[table-format=-1.6] S[table-format=-1.6] S[table-format=-1.6]}\n');
fprintf(fid, '        \\toprule\n\n');
...
for ll = 1:ii % ii is counter inside loop above (counting valid table rows), ll is index of resulting CSM_data, mm is index of table data from Rogers 1992
    mm = kk(ll);
%     if gw(mm) == 1
%         fprintf(fid, '         \\multicolumn{1}{l}{%s} & \\multicolumn{6}{l}{Solution to the Falkner-Skan equations, see Table \\ref{tab:FSF}}   \\\\\n',VerData_Rogers1992_TableC27(mm,1));
%     elseif Beta_dact(mm) == 2.0
    if Beta_dact(mm) == 2.0 %%% note that this version is changed compared to C-25 and C-26: with Ma>0 string-replace is used to print scientific notation (for small and large m2)
%         if sep(ll) == 1 % this solution is more difficult to find
%             fprintf(fid, '         %s      &   %s       &   %4.0f\\tnote{*}          &  %s   &  \\multicolumn{1}{c}{sep}   &  %s   &  \\multicolumn{1}{c}{sep}  \\\\\n',VerData_Rogers1992_TableC27(mm,1),VerData_Rogers1992_TableC27(mm,2),PGm2(mm),VerData_Rogers1992_TableC27(mm,3),VerData_Rogers1992_TableC27(mm,4));
%         else
%             fprintf(fid, '         %s       &   %s       &   %4.0f\\tnote{*}          &  %s   &  %1.6f   &  %s   &  %1.6f  \\\\\n',VerData_Rogers1992_TableC27(mm,1),VerData_Rogers1992_TableC27(mm,2),PGm2(mm),VerData_Rogers1992_TableC27(mm,3),CSM_DataTableC27(ll,1)*sqrt(2*FLD.C/(PGm2(mm) + 1)),VerData_Rogers1992_TableC27(mm,4),CSM_DataTableC27(ll,2)*sqrt(2*FLD.C/(PGm2(mm) + 1)));
%         end
        if sep(ll) == 1 % this solution is more difficult to find
            fprintf(fid, '         %s      &   %s       &   %s\\tnote{*}          &  %s   &  \\multicolumn{1}{c}{sep}   &  %s   &  \\multicolumn{1}{c}{sep}  \\\\\n',VerData_Rogers1992_TableC27(mm,1),VerData_Rogers1992_TableC27(mm,2),num2str(PGm2(mm),'%1.6e'),VerData_Rogers1992_TableC27(mm,3),VerData_Rogers1992_TableC27(mm,4));
        else
            fprintf(fid, '         %s       &   %s       &   %s\\tnote{*}          &  %s   &  %1.6f   &  %s   &  %1.6f  \\\\\n',VerData_Rogers1992_TableC27(mm,1),VerData_Rogers1992_TableC27(mm,2),num2str(PGm2(mm),'%1.6e'),VerData_Rogers1992_TableC27(mm,3),CSM_DataTableC27(ll,1)*sqrt(2*FLD.C/(PGm2(mm) + 1)),VerData_Rogers1992_TableC27(mm,4),CSM_DataTableC27(ll,2)*sqrt(2*FLD.C/(PGm2(mm) + 1)));
        end
    elseif sep(ll) == 1 % separation has occurred, print 'sep'
%         fprintf(fid, '                  &   %s       &   %1.6f            &  %s   &  \\multicolumn{1}{c}{sep}   &  %s   &  \\multicolumn{1}{c}{sep}  \\\\\n',VerData_Rogers1992_TableC27(mm,2),PGm2(mm),VerData_Rogers1992_TableC27(mm,3),VerData_Rogers1992_TableC27(mm,4));
        fprintf(fid, '                  &   %s       &   %s            &  %s   &  \\multicolumn{1}{c}{sep}   &  %s   &  \\multicolumn{1}{c}{sep}  \\\\\n',VerData_Rogers1992_TableC27(mm,2),num2str(PGm2(mm),'%1.6e'),VerData_Rogers1992_TableC27(mm,3),VerData_Rogers1992_TableC27(mm,4));
    else
%         fprintf(fid, '                  &   %s       &   %1.6f            &  %s   &  %1.6f   &  %s   &  %1.6f  \\\\\n',VerData_Rogers1992_TableC27(mm,2),PGm2(mm),VerData_Rogers1992_TableC27(mm,3),CSM_DataTableC27(ll,1)*sqrt(2*FLD.C/(PGm2(mm) + 1)),VerData_Rogers1992_TableC27(mm,4),CSM_DataTableC27(ll,2)*sqrt(2*FLD.C/(PGm2(mm) + 1)));
%         exp_format = strrep(num2str(PGm2(2),'%1.5e\n'), 'e-0','e-');
        fprintf(fid, '                  &   %s       &   %s            &  %s   &  %1.6f   &  %s   &  %1.6f  \\\\\n',VerData_Rogers1992_TableC27(mm,2),num2str(PGm2(mm),'%1.6e'),VerData_Rogers1992_TableC27(mm,3),CSM_DataTableC27(ll,1)*sqrt(2*FLD.C/(PGm2(mm) + 1)),VerData_Rogers1992_TableC27(mm,4),CSM_DataTableC27(ll,2)*sqrt(2*FLD.C/(PGm2(mm) + 1)));
    end
end %%% strrep(num2str(PGm2(2),'%1.5e\n'), 'e-0','e-') %%%
% fprintf(fid, '        %% Data\n');
...
%% Table C-27 extended (J1, J2) - Write data to Tex-file (in Table format) #1

% NewFolder = cd;
% cd('Thesis_Report_Tables') %%% new, added (21-01-2021)
fid = fopen('Verification_Rogers1992_TableC27_extended.tex','w');
fprintf(fid, '%%%%%%%%%% TABLE PRINTED BY MATLAB PROGRAM %%%%%%%%%%\n');
fprintf(fid, '%%%% Verification with Table C-27 from Rogers\\cite{rogers1992laminar}: Pr=1, C=1 (constant)\n');
% fprintf(fid, '%%\\begin{table}\n');
% fprintf(fid, '%%\\begin{sidewaystable}\n');
fprintf(fid, '\\begin{threeparttable}\n');
fprintf(fid, '    \\centering\n');
fprintf(fid, '    \\caption{Comparison with tabulated data of laminar compressible similar flows with constant nonzero pressure gradients and heat transfer for calorically perfect ideal gas with $C = 1$ (constant) and $\\mathrm{Pr} = 0.723$ and $\\bar{\\sigma} = 2.0 \\quad (\\mathrm{Ma} = \\infty)$ taken from Rogers\\cite{rogers1992laminar} table C-27. The values obtained with the CS-method (CSM) are transformed from the compressible Falkner-Skan transformed y-coordinate with uniform (vertical) grid spacing of $\\mathrm{d} \\eta = \\sqrt{\\frac{2C}{m_2 + 1}} %1.4f$ and height of $\\eta_{\\mathrm{e}} = \\sqrt{\\frac{2C}{m_2 + 1}} %1.1f$ to the Illingworth-Levy coordinates ($\\mathrm{d} \\eta = %1.4f$ and $\\eta_{\\mathrm{e}} = %1.1f$). Note that separation occurred when the table entry shows ''sep''.}\n',GRD.Deta(1),GRD.etaE,GRD.Deta(1),GRD.etaE);
fprintf(fid, '    \\label{tab:C27E}\n');
fprintf(fid, '    \\begin{tabular}{S[table-format=1.1] S[table-format=-1.6] S[table-format=-1.6e-1] S[table-format=-1.6] S[table-format=-1.6] S[table-format=-1.6] S[table-format=-1.6] S[table-format=1.6] S[table-format=1.6] S[table-format=1.6] S[table-format=1.6]}\n');
%printf(fid, '    \\begin{tabular}{S[table-format=1.1] S[table-format=-1.6] S[table-format=-1.6e-1] S[table-format=-1.6] S[table-format=-1.6] S[table-format=-1.6] S[table-format=-1.6]}\n');
fprintf(fid, '        \\toprule\n\n');
fprintf(fid, '        \\multicolumn{1}{c}{$g_{\\mathrm{w}}$}                     &\n');
fprintf(fid, '        \\multicolumn{1}{c}{$\\hat{\\beta}$}                         &\n');
fprintf(fid, '        \\multicolumn{1}{c}{$m_2$}                                 &\n');
...
%     elseif Beta_dact(mm) == 2.0
    if Beta_dact(mm) == 2.0
        if sep(ll) == 1 % this solution is more difficult to find
            fprintf(fid, '         %s      &   %s       &   %s\\tnote{*}          &  %s   &  \\multicolumn{1}{c}{sep}   &  %s   &  \\multicolumn{1}{c}{sep}   &  %s   &  \\multicolumn{1}{c}{sep}   &  %s   &  \\multicolumn{1}{c}{sep}  \\\\\n',VerData_Rogers1992_TableC27(mm,1),VerData_Rogers1992_TableC27(mm,2),num2str(PGm2(mm),'%1.6e'),VerData_Rogers1992_TableC27(mm,3),VerData_Rogers1992_TableC27(mm,4),VerData_Rogers1992_TableC27(mm,5),VerData_Rogers1992_TableC27(mm,6));
        else
            fprintf(fid, '         %s       &   %s       &   %s\\tnote{*}          &  %s   &  %1.6f   &  %s   &  %1.6f   &  %s   &  %1.6f   &  %s   &  %1.6f  \\\\\n',VerData_Rogers1992_TableC27(mm,1),VerData_Rogers1992_TableC27(mm,2),num2str(PGm2(mm),'%1.6e'),VerData_Rogers1992_TableC27(mm,3),CSM_DataTableC27(ll,1)*sqrt(2*FLD.C/(PGm2(mm) + 1)),VerData_Rogers1992_TableC27(mm,4),CSM_DataTableC27(ll,2)*sqrt(2*FLD.C/(PGm2(mm) + 1)),VerData_Rogers1992_TableC27(mm,5),CSM_DataTableC27(ll,3)*2/sqrt(2*FLD.C/(PGm2(mm) + 1)),VerData_Rogers1992_TableC27(mm,6),CSM_DataTableC27(ll,4)*2/sqrt(2*FLD.C/(PGm2(mm) + 1)));
        end
%     if Beta_dact(mm) == 2.0
%         fprintf(fid, '         %s       &   %s       &   %4.0f\\tnote{*}          &  %s   &  %1.6f   &  %s   &  %1.6f   &  %s   &  %1.6f   &  %s   &  %1.6f  \\\\\n',VerData_Rogers1992_TableC27(mm,1),VerData_Rogers1992_TableC27(mm,2),PGm2(mm),VerData_Rogers1992_TableC27(mm,3),CSM_DataTableC27(ll,1)*sqrt(2*FLD.C/(PGm2(mm) + 1)),VerData_Rogers1992_TableC27(mm,4),CSM_DataTableC27(ll,2)*sqrt(2*FLD.C/(PGm2(mm) + 1)),VerData_Rogers1992_TableC27(mm,5),CSM_DataTableC27(ll,3)/sqrt(2*FLD.C/(PGm2(mm) + 1)),VerData_Rogers1992_TableC27(mm,6),CSM_DataTableC27(ll,4)/sqrt(2*FLD.C/(PGm2(mm) + 1)));
    elseif sep(ll) == 1 % separation has occurred, print 'sep'
        fprintf(fid, '                  &   %s       &   %s            &  %s   &  \\multicolumn{1}{c}{sep}   &  %s   &  \\multicolumn{1}{c}{sep}   &  %s   &  \\multicolumn{1}{c}{sep}   &  %s   &  \\multicolumn{1}{c}{sep}  \\\\\n',VerData_Rogers1992_TableC27(mm,2),num2str(PGm2(mm),'%1.6e'),VerData_Rogers1992_TableC27(mm,3),VerData_Rogers1992_TableC27(mm,4),VerData_Rogers1992_TableC27(mm,5),VerData_Rogers1992_TableC27(mm,6));
    else
        fprintf(fid, '                  &   %s       &   %s            &  %s   &  %1.6f   &  %s   &  %1.6f   &  %s   &  %1.6f   &  %s   &  %1.6f  \\\\\n',VerData_Rogers1992_TableC27(mm,2),num2str(PGm2(mm),'%1.6e'),VerData_Rogers1992_TableC27(mm,3),CSM_DataTableC27(ll,1)*sqrt(2*FLD.C/(PGm2(mm) + 1)),VerData_Rogers1992_TableC27(mm,4),CSM_DataTableC27(ll,2)*sqrt(2*FLD.C/(PGm2(mm) + 1)),VerData_Rogers1992_TableC27(mm,5),CSM_DataTableC27(ll,3)/sqrt(2*FLD.C/(PGm2(mm) + 1)),VerData_Rogers1992_TableC27(mm,6),CSM_DataTableC27(ll,4)/sqrt(2*FLD.C/(PGm2(mm) + 1)));
    end
end
% fprintf(fid, '        %% Data\n');
fprintf(fid, '        \\bottomrule\n\n');
